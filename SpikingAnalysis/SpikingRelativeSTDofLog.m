function SpikingRelativeSTDofLogData = SpikingRelativeSTDofLog(binwidthsecs,smoothingnumpoints,basepath,basename)
...
RSDI = std(log10(IBinRates,[],2));
RSDI = RSDI./mean(RSDI);

%% Smooth
RSDE = smooth(RSDE,smoothingnumpoints);
RSDI = smooth(RSDI,smoothingnumpoints);
for a = 1:numEcells
...
if plotting
    ...
    subplot(6,1,5);
    plot(bincentertimes,RSDE,'b')
    axis tight
    plotIntervalsStrip(gca,stateintervals,1)

    subplot(6,1,6);
    plot(bincentertimes,RSDI,'r')
    axis tight
    plotIntervalsStrip(gca,stateintervals,1)
end
% 
...
SpikingRelativeSTDofLogData = v2struct(RSDE,RSDI,EBinRates,IBinRates,binwidthsecs,bincentertimes,smoothingnumpoints,numEcells,numIcells);
save(fullfile(basepath,[basename '_SpikingRelativesSTDofLogData.mat']),'SpikingRelativesSTDofLogData')