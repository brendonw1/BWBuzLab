function SpikingCoeffVarationData = SpikingCoeffVariation(binwidthsecs,smoothingnumpoints,basepath,basename)
...
%% Smooth
CoVE = smooth(CoVE,smoothingnumpoints);
CoVI = smooth(CoVI,smoothingnumpoints);
for a = 1:numEcells
...
if plotting
    ...
    subplot(6,1,5);
    plot(bincentertimes,CoVE,'b')
    axis tight
    plotIntervalsStrip(gca,stateintervals,1)

    subplot(6,1,6);
    plot(bincentertimes,CoVI,'r')
    axis tight
    plotIntervalsStrip(gca,stateintervals,1)
end
% 
...
SpikingCoeffVarationData = v2struct(CoVE,CoVI,EBinRates,IBinRates,binwidthsecs,bincentertimes,smoothingnumpoints,numEcells,numIcells);
save(fullfile(basepath,[basename '_SpikingCoeffVaration.mat']),'SpikingCoeffVarationData')