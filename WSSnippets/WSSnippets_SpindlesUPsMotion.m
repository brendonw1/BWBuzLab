function UPSpindleMotionWSSnippets = WSSnippets_SpindlesUPsMotion(ep1,ep2)
...
[preUPs,postUPs,tep1,tep2] = VectorsFromEpochPairs(upstartsarr,ep1,ep2);
for a = 1:length(tep1)
...
[preUPDur,postUPDur] = VectorsFromEpochPairs(updurarr,ep1,ep2);
...
[preUPSpkRate,postUPSpkRate] = VectorsFromEpochPairs(upspkrtarr,ep1,ep2);% upcountarr = cat(2,upstarts,updurations);
...
[preSpindles,postSpindles,tep1,tep2] = VectorsFromEpochPairs(spindlestartsarr,ep1,ep2);% upcountarr = cat(2,upstarts,updurations);
for a = 1:length(tep1)
    tprespan = tot_length(tep1{a},'s');
...
[preSpindleDur,postSpindleDur] = VectorsFromEpochPairs(spdurarr,ep1,ep2);% upcountarr = cat(2,upstarts,updurations);
...
[preSpindleFreq,postSpindleFreq] = VectorsFromEpochPairs(spfreqarr,ep1,ep2);% upcountarr = cat(2,upstarts,updurations);
...
[preSpindleAmp,postSpindleAmp] = VectorsFromEpochPairs(spamparr,ep1,ep2);% upcountarr = cat(2,upstarts,updurations);
...
[preSpindleSpkRate,postSpindleSpkRate] = VectorsFromEpochPairs(spspkrtarr,ep1,ep2);% upcountarr = cat(2,upstarts,updurations);
