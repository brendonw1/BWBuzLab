...
%     disp(a);
    thisshank =  shank(a);
    thisid = cellIx(a);
    disp(['Starting Cell ',num2str(a), ' out of ', num2str(size(S,1))]);
...
%% USING this version since calcClusterQuality needs the full fet (also will be faster as long as ok for ram)
    if loadedshanks~=thisshank
       disp(['Starting Shank ',num2str(thisshank)]);
       [thisfet,dummy] = LoadFet([basename,'.fet.',num2str(thisshank)]);
    ...
    Energies{a} = (sum(thiscellfet2.^2,2)).^.5;
        
    mahald = mahal(thiscellfet,thiscellfet);
    SelfDistances {a} = mahald;
    ...
    LRatios(a,1:5) = [Lratio_all Lratio1 Lratio2 Lratio3 Lratio4];
    IsoDistances(a,1:5) = [ID_all ID1 ID2 ID3 ID4];
    ISIIndices(a,1:5) = [ISIindex_all ISIindex1 ISIindex2 ISIindex3 ISIindex4];
end
