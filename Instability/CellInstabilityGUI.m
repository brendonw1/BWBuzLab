function CellInstabilityGUI(basepath,basename)
...
binned = Data(MakeQfromS(S,binsamps));
bintimes = ((1:size(binned,1))*binwidth)-binwidth/2;
numcells = length(S);
...
if exist([basename '_CellSpikeTimesEnergies.mat'],'file') %if already did this, then load stored version
    t = load([basename '_CellSpikeTimesEnergies.mat']);
    celltimes = t.celltimes;
    cellenergies = t.cellenergies;
    ...
    for tcnum = 1:length(shank); %store spike data for each cell
        ...
        ttimes = tfet(tspikeidxs,end)/par.SampleRate;
        tenergies = sum(tfet(tspikeidxs,1:tnfets),2);
    ...
    save([basename '_CellSpikeTimesEnergies'],'celltimes','cellenergies')
end
...
if exist([basename '_ShankwiseAutoInstabilities.mat'],'file') %if already did this, then load stored version
    ...
    for a = 1:length(allusedshanks)
        ...
        [shankinstabilityints{a},cellpastthreshbools{a}] = CellShankInstability(basename,tshank);
...
handles.Data = v2struct(shank,cellIx,allusedshanks, binned, bintimes,...
    celltimes, cellenergies,...
...
scalefact = max(handles.Data.cellenergies{newclu})/max(handles.Data.binned(:,newclu))/2;
shiftfact = max(handles.Data.cellenergies{newclu});
handles.spikebinlines = line(handles.Data.bintimes,scalefact*handles.Data.binned(:,newclu)+shiftfact,...
