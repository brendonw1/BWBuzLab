...
[slope, intercept, MSE, r, SlopeCI, p, S, yfit] = logfit_bw(v1,v2,plotmode,'color',color);
if isempty(SlopeCI)
    SlopeCI = nan;
end
...
%% plotting idenity line
        switch lower(idlinemode)
            case 'linear'
                plot([mimi mama],[0 mama],'color',[.5 .5 .5])
            case 'logx'
                mimi = min([v1(v1>0);v2(v2>0)]);
                semilogx([mimi mama],[0 mama],'color',[.5 .5 .5])
            case 'logy'
                mimi = min([v1(v1>0);v2(v2>0)]);
                semilogy([0 mama],[mimi mama],'color',[.5 .5 .5])
            case 'loglog'
                mimi = min([v1(v1>0);v2(v2>0)]);
