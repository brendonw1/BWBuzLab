...
%     subplot(2,2,4) %loglog
        [slope, intercept,MSE, R, SlopeCI, P, S] = logfit_bw(x,y,'loglog');
        ...
        try
            th = text(0.7*max(x),0.8*max(y),{['r=',num2str(R)];['p=',num2str(P)];...
                ['slope = ' num2str(slope)];['SlopeCI = ' num2str(SlopeCI(1)) '-' num2str(SlopeCI(2))]});
        end
        if P<=0.05;set(th,'color',[1 0 0]);end
%         xl = log10(x);
%         xl = real(xl);
