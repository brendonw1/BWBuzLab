function [ INT, IDX ] = StateID(LFP,thLFP,EMG,sf_LFP,sf_EMG,figloc,WSEpisodes)
%StateID(LFP,thLFP,EMG,sf_LFP,sf_EMG,figloc,WSEpisodes)
%   Detailed explanation goes here
%
%
%Last Updated: 9/13/15
%DLevenstein
...
minSWS = 5;
minWnexttoREM = 10;    %ONLY APPLIES TO NONMOVING WAKE
minWinREM = 10;       %ONLY APPLIES TO NONMOVING WAKE
minREMinW = 20;
minREM = 5;
minWAKE = 5;
...
%% Divide PC1 for SWS
[pcahist,histbins]= hist(SCORE(:,1),10);
...
figure
    subplot(1,3,[2,3])
        scatter3(SCORE(:,1),thratio,EMG,5,IDX,'filled')
        %rotate3d
        view(133.7,18.8);
        grid on
	...
	subplot(3,3,4)
        ...
        xlabel('EMG')
        title('Step 2: EMG for Muscle Activity')
	subplot(3,3,7)
        hold on
        bar(THhistbins,THhist)
