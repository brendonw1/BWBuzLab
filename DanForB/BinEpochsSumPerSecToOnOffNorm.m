function [on_binned, off_binned, norm_binned, onbins, offbins, normbins] = ...
    BinEpochsSumPerSecToOnOffNorm(epochdatapoints, epochsamptimes, int, sampfreq, tbins4norm, sampoverhangs)
...
for a=1:ne
    ...
    on_binned(:,a) = BinEpochsSumPerSec(epochdatapoints{a}, epochsamptimes{a}, tonbins);
    on_binned(:,a) = on_binned(:,a);%divide by bin width
    
    off_binned(:,a) = BinEpochsSumPerSec(epochdatapoints{a}, epochsamptimes{a}, toffbins);
    off_binned(:,a) = off_binned(:,a);%divide by bin width
    
    norm_binned(:,a) = BinEpochsSumPerSec(epochdatapoints{a}, epochsamptimes{a}, tnormbins);
    norm_binned(:,a) = norm_binned(:,a);%divide by bin width
end
