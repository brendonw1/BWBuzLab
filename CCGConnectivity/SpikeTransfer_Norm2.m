function [strengthbyratio, strengthbyratechg, ccg, measured, expected] = SpikeTransfer_Norm(times1,times2,binSize,duration,sigbinms,ConvWidth,sameshank,SampleRate)
...
% ccg = cross correlogram from which things are calculated
% meausured = sum of counts in designated correlation bins, in Hz
% expected = sum of expected counts in same bins, in Hz
% 
% Optional OUTPUT
...
%     clu = [ones(size(s1))' 2*ones(size(s2))'];
    ...
    if sameshank% if same shank, do more bins and also smooth wider, but with the same bin width
        ...
        [~, pred, ~ ] = cch_conv(round(ccgR(:,1,2)), ConvWidth2);
    ...
    %     cchCor(partialBins,:) = ccgR(partialBins,:).*repmat(correctionRatio',[1 size(cch,2)]);
        ...
        tR = tR/1000;%convert from ms, for some reason that is default output
        %[dummy, pred, dummy ] = cch_conv(round(ccgR(:,1,2)),abs(ConvWidth));
        [~, pred, ~ ] = cch_conv(round(ccgR(:,1,2)),round(ConvWidth));
        ccg = ccgR(:,1,2);
    end

    sigbins = sort(sigbinms/1000);
    gb = find(tR>=sigbins(1),1,'first'):find(tR<=sigbins(2),1,'last');
...
% RB use CCGHeart instead of CCGFun and feval
...
if nargout>=3
 ...
 %        [Counts RawPairs] = CCGHeart(Res, uint32(Clu), BinSize, uint32(HalfBins));
     %direct call to CCGHeart without feval RB
...
rsRawPairs = reshape(RawPairs, [2 length(RawPairs)/2])';
 ...
 %    http://www.mathworks.com/support/compilers/current_release    Counts = CCGHeart(Res, uint32(Clu), BinSize, uint32(HalfBins)); %direct call to CCGHeart without feval RB
    end 
else
    Counts = feval(CCGFun,Res, uint32(Clu), BinSize, uint32(HalfBins));
